...
@dataclass
class KofNSystem(BaseSystem):
    k: int = 1  # minimum working components
    ...
    def components_to_repair(self):
        # Repair enough failed components to restore k-of-n
        working = sum(c.state == 1 for c in self.components)
        needed = max(0, self.k - working)
        failed = [c for c in self.components if c.state == 0]
        return failed[:needed]

    def __repr__(self):
        return f"{self.__class__.__name__}(name={self.name}, state={self.state}, k={self.k}, components={len(self.components)})"
    ...
    def R_s(self, t=None):
        ...
        if t is None or isinstance(t, sp.Symbol):
            ...
            for comb in combinations(self.components, self.k):
                term = 1
                for c in comb:
                    term *= c.R_t(t_sym)
                R_sym += term
