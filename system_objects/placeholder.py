...
#         ax.set_ylabel("State")
    ...
    def repair(self, dt: float = 1.0):
        ...
        # max repair time 
        repair_duration = max(rt for _, rt in repair_times)
        print(f"Repairing {len(comps)} components, max duration: {repair_duration:.2f}")
        ...
        while any(rt > 0 for _, rt in repair_times):
            ...
            for i, (comp, remaining) in enumerate(repair_times):
                if remaining > 0:
                    ...
                    # step the components not being repaired
                    for c in self.components:
                        if c.state != REPAIR_STATE:
                            c.step(dt)
        ...
        # Update the final state of the components and the system to working after repair
        ...
        self.state = self.structure_function()
        self.history = np.vstack([self.history, [self.current_time, self.state]])
        self.current_time += self.current_time + repair_duration
